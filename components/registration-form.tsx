...
export function RegistrationForm() {
  ...
  function validate(): boolean {
    ...
    if (!/^\d{10}$/.test(form.phoneNumber.trim())) errors.phoneNumber = 'Enter a valid 10-digit number'
    if (!/^\d{10}$/.test(form.gurdianPhoneNumber.trim())) errors.gurdianPhoneNumber = 'Enter a valid 10-digit number'
    ...
    if (!/^\d{6}$/.test(form.pin.trim())) errors.pin = 'Enter a valid 6-digit pincode'
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!scriptLoaded) {
        stopSubmitting('Could not load the payment gateway. Check your connection and try again.')
        return
      }
      ...
      if (!orderRes.ok || !orderData.success) {
        stopSubmitting(orderData.message || 'Could not initiate payment. Please try again.')
        return
      }
      ...
      watchdogRef.current = setTimeout(() => {
        stopSubmitting('The payment window did not open. Please check your connection and try again.')
      }, 8000)
      ...
      razorpay.on('payment.failed', (resp: any) => {
        stopSubmitting(resp?.error?.description || 'Payment failed. Please try again.')
      })
    ...
    <div className="rounded-2xl border border-border bg-card p-6 sm:p-8">
      ...
      <form onSubmit={handleSubmit} className="mt-6 space-y-4">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <Field label="Guardian Phone Number" error={fieldErrors.gurdianPhoneNumber}>
            <input
              type="tel"
              value={form.gurdianPhoneNumber}
              onChange={(e) => updateField('gurdianPhoneNumber', e.target.value.replace(/\D/g, ''))}
        ...
        <Field label="School Board (Class 12th)" error={fieldErrors.schoolBoardName}>
          <input
            type="text"
        ...
        <Field label="Permanent Address" error={fieldErrors.permanentAddress}>
          <textarea
            ...
            className={inputClass(!!fieldErrors.permanentAddress) + ' min-h-[80px] resize-none'}
        ...
        <div className="grid gap-4 sm:grid-cols-3">
          <Field label="Pincode" error={fieldErrors.pin}>
            ...
            {pinLookupStatus === 'loading' && (
              <p className="mt-1 text-xs text-muted-foreground">Looking up location…</p>
            )}
            {pinLookupStatus === 'not-found' && (
              <p className="mt-1 text-xs text-muted-foreground">Couldn't auto-detect. Enter state/district manually.</p>
            )}
          </Field>
